...
class AffinityPDF(object):
    """
    Represents an Affinity PDF monthly statement.
    """
    ...
    def __init__(self, path: str) -> None:
        ...
        self.month = None  # the month of the statement as a str (e.g. "Jan", "Feb")
        self.date = None  # the date of the statement as a str (e.g. "31", "29")

        self.__date_column = None  # the x0 and x1 of the date column
        self.__deposit_column = None  # the x0 and x1 of the deposit column
        self.__description_column = None  # the x0 and x1 of the description column
                ...
                if isinstance(element, LTTextBoxHorizontal):
                    ...
                    if (
                        ...
                        self.closing_balance = (
                            datetime.strptime(
                                f"{self.month} {self.date} {self.year}", "%b %d %Y"
                            ),
                            float(text),
                        )
    ...
    def populate_transactions(self) -> None:
        ...
        if (
            ...
            or not self.__description_column
        ...
        for i, page_layout in enumerate(self.read_pages(self.__path)):
            for element in page_layout:
                if isinstance(element, LTTextBoxHorizontal):
                    ...
                    if (
                        abs(self.__description_column[0] - element.x0) < TRANSACTION_X0
                        and abs(self.__description_column[1] - element.x1)
                        < TRANSACTION_X1
                    ):
