...
long long countSolves(std::vector<long long> unsolved, long long result)
{
    ...
    // call unglue()
    if (unglue(result, unsolved[unsolved.size() - 1]) != -1)
    {
        count += countSolves(std::vector<long long>(unsolved.begin(), unsolved.end() - 1), unglue(result, unsolved[unsolved.size() - 1]));
    }
